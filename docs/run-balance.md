...
|---|---|---|---|---|
| Cautious | 23.0% | 45.7% | 50.4% | 26% |
| Balanced | 21.3% | 44.0% | 48.5% | 26% |
| Greedy | 21.0% | 43.0% | 48.8% | 23% |

A won hallway costs 12 HP (19% of max), a won elite 16 HP (25%). Decks reach the boss at a
...
| 5 | Ash Hound escalation softened (Howl +3 → +1 Strength, Maul 14 → 12) | 44% | 21% |
...
**Pass 5 — soften the escalation, not the fight.** Ash Hound appears only in the elite, and it
was the killer: Brace (13 Block) stretched the fight while every Howl stacked +3 Strength, so
Maul climbed 14 → 17 → 20. Howl now grants +1 and Maul starts at 12; the hound's health is
unchanged, so an elite is still a long and costly fight. Same seeds, before → after:

| policy | run win | reach boss | win if reached | died in elites |
|---|---|---|---|---|
| Hunter | 17.0% → **22.0%** | 28.7% → 37.0% | 59.3% → 59.5% | 108 → 86 |
| Avoider | 22.3% → 23.3% | 44.3% → 47.0% | 50.4% → 49.6% | 27 → 22 |

The gap in run wins closed from 5.3 points to 1.3, well inside the ~3.4-point standard error of a
difference between two such rates. Elites moved from a clear losing bet to roughly break-even:
...
- **Elites are break-even, not yet a good bet.** After pass 5, hunting elites wins 22.0% of runs
  against 23.3% for avoiding them — a difference inside the noise. The design intent is a good bet
  for a healthy deck, which would mean Hunter clearly ahead. Elites are still where Hunter dies
  most (86 of its 234 losses). The next lever is what follows an elite — a guaranteed rest on the
  row after one — rather than another cut to the fight itself, which would start to make elites
  trivial.
