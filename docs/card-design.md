...
Every card has one upgrade, and the rest site is where you get it: **heal 30% of max HP, or upgrade
one card.** A rest that only heals is not a decision; heal-or-upgrade asks whether the next fights
are a question of surviving them or of being stronger for them. The picker shows each card *as it
will be after upgrading*, so the comparison happens on screen rather than in the player's head.

**An upgrade is a separate card, `strike` → `strike_plus`.** The run deck holds card definitions,
so upgrading swaps one copy for its "+" definition. Effects, descriptions, rewards and saves then
work for upgraded cards with no special case. (The original plan put an upgraded flag on
`CardInstance`; instances only live for one combat, so that would have needed a parallel
run-level structure for no gain.) Upgraded cards are in `AllCards`, so saves can name them, and
kept out of `RewardPool`, so rewards never offer them.

**Upgrades are generated by rule, not written sixty times.** New cards get one automatically and
all of them stay consistent:

| what the card has | upgrade |
|---|---|
| damage or Block | +50%, between +2 and +8 (multi-hit: +1 per hit) |
| a status | +1; Burn +⅓ of the stack, at least 2 |
| draw / heal / Heat | +1 / +3 / +1 |
| a multiplier or ratio | +0.5 |
| "X per card" | +⅓, at least 1 |
| Power | cost −1 |
| none of the above | cost −1, or draw a card if it already costs 0 |

The rule produced 21 cost reductions and 4 fallbacks across the sixty. **EmberDeck → Audit
Upgrades** prints every card beside its upgrade, and the generator fails loudly on an upgrade
identical to its base. That check found a real text bug on its first run: Immolate dealt 1.5x Burn
while its text said "equal to Burn", so its 2x upgrade printed the same sentence. Reading the
pairs also caught two upgrades that were technically better but not worth a rest (Ash Armor 10 →
11 per card, Wildfire 8 → 10 Burn), which is why those two rules are proportional.

**Balance effect: within noise, for now.** The simulator's rest policy heals below 70% HP and
otherwise upgrades its rarest card. It upgraded only about 0.2 times per run: bots rest rarely and
usually arrive hurt. Run wins moved less than two points for every policy (Cautious 23.0 → 23.7,
Hunter 22.0 → 21.7, Avoider 23.3 → 22.3), inside the ~2.4-point standard error. A person who
routes to rests while healthy will upgrade far more often than the bot, so this is a floor, not
a measurement of what upgrades are worth.
